...
def parse_node_pfn_stats(filepath='/sys/kernel/debug/numa_folio/pfn_stats'):
    """Parse PFN ranges for each NUMA node from the provided debug file."""
    node_ranges = {}
    try:
...
def combine_logs_and_generate_heatmaps(log_pattern="folio_logs/folio_stats_snapshot_*.log",
    ...
    """Combine multiple log files and generate heatmaps showing folio migration counts."""
    log_files = sorted(
    ...
    line_regex = re.compile(
        r"folio node:?\s*(\d+),\s*pfn:?\s*(\d+),\s*source_nid:?\s*(\d+),\s*migrate_count:?\s*(\d+)"
    )
    ...
    for node, (start_pfn, end_pfn) in node_ranges.items():
        ...
        if not node_df.empty:
            pivot = node_df.pivot_table(
                ...
                aggfunc="sum",
                fill_value=0
            ).reindex(index=full_pfn_range, fill_value=0)
        else:
...
def main():
    # Import Dask Client only when main() is run
    ...
    parser.add_argument("--log_pattern", type=str,
                        default="folio_logs/folio_stats_snapshot_*.log",
                        help="Glob pattern for log files (default: folio_logs/folio_stats_snapshot_*.log)")
    parser.add_argument("--node_pfn_stats", type=str,
                        default="/sys/kernel/debug/numa_folio/pfn_stats",
