...
def main():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "its PID via syscall 463, and collects data from /sys/kernel/debug/numa_folio/folio_stats "
            "every interval seconds. Each snapshot is saved as a separate log file."
        )
    )
    parser.add_argument(
        "command",
    ...
    # 워크로드 실행 후 잠깐 대기한 후, 해당 pid를 syscall 463로 전달
    time.sleep(0.1)
    send_pid_to_syscall(proc.pid)

    print("Starting data collection. Each snapshot will be saved in a separate file...")
    snapshot = 0
    try:
        while proc.poll() is None:
            ...
            # 타임스탬프를 파일 이름에 활용하기 위해 포맷
            timestamp = time.strftime("%Y-%m-%d_%H-%M-%S", time.localtime())
            filename = f"folio_stats_{timestamp}_snapshot_{snapshot}.txt"

            with open(filename, "w") as log_file:
                log_file.write(f"Snapshot {snapshot} at {timestamp}\n")
                log_file.write("=" * 80 + "\n")
                log_file.write(data)
